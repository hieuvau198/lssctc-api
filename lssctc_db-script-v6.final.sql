...
-- =========================
-- Program & Course area (base lookups, courses, programs, sections, activities, materials)
-- =========================
--  Base lookup tables
-- ========================================
...
-- =========================
-- Classes & Enrollments & Tracking (from sql.txt)
-- =========================
...
CREATE TABLE program_course_classes (
    Id INT IDENTITY(1,1) PRIMARY KEY,
    ProgramCourseId INT NOT NULL,
       FOREIGN KEY (ProgramCourseId) REFERENCES pm.program_courses(Id)
);
...
CREATE TABLE [dbo].[practice_attempts] (
    ...
    [score_obtained] DECIMAL(10,2) NULL,
    [max_score] DECIMAL(10,2) NULL,
    [status] NVARCHAR(50) NOT NULL DEFAULT N'InProgress', 
...
CREATE TABLE [dbo].[practice_attempt_tasks] (
    ...
    [score_obtained] DECIMAL(10,2) NULL,       -- điểm riêng cho task
    [max_score] DECIMAL(10,2) NULL,
    [status] NVARCHAR(50) NOT NULL DEFAULT N'NotStarted', 
...
-- =========================
-- Practices & Tasks (from Script.txt)
-- =========================
--  Practices and tasks
-- ========================================
...
SELECT v.name FROM (VALUES ('CL000001'),('CL000002'),('CL000003'),('CL000004'),('CL000005')) AS v(name)
WHERE NOT EXISTS (SELECT 1 FROM [dbo].[class_codes] c WHERE c.[name] = v.name);
...
WHERE NOT EXISTS (SELECT 1 FROM [dbo].[course_levels] cl WHERE cl.[name] = v.name);

INSERT INTO [dbo].[course_codes] ([name])
SELECT v.name FROM (VALUES ('CO000001'),('CO000002'),('CO000003'),('CO000004'),('CO000005')) AS v(name)
WHERE NOT EXISTS (SELECT 1 FROM [dbo].[course_codes] cc WHERE cc.[name] = v.name);
